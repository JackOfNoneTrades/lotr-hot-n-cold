...
public class Config {
    ...
    public static boolean customizeSpawnEggLOTREquipment = false;
    public static boolean customizeStructureLOTREquipment = false;
    public static boolean customizeInvasionLOTREquipment = false;
    ...
    public static String[] lotrNPCGroupMembers = {};
    ...
    private static void readNPCEquipmentConfiguration(Configuration configuration) {
        customizeSpawnEggLOTREquipment = configuration.getBoolean(
            "customizeSpawnEggLOTREquipment",
            Configuration.CATEGORY_GENERAL,
            false,
            "Apply equipment rules to new LOTR spawn-egg NPCs, including dispenser eggs. NPC protection settings still apply; LOTR eggs mark NPCs persistent.");
        customizeStructureLOTREquipment = configuration.getBoolean(
            "customizeStructureLOTREquipment",
            Configuration.CATEGORY_GENERAL,
            false,
            "Apply equipment rules to new NPCs generated by LOTR structures and their NPC respawners. NPC protection settings still apply; structure NPCs are normally persistent.");
        customizeInvasionLOTREquipment = configuration.getBoolean(
            "customizeInvasionLOTREquipment",
            Configuration.CATEGORY_GENERAL,
            false,
            "Apply equipment rules to new NPCs created by LOTR invasion spawners. NPC protection settings still apply; invasion NPCs are normally persistent.");
        logLOTREquipmentChanges = configuration.getBoolean(
            "logLOTREquipmentChanges",
            Configuration.CATEGORY_GENERAL,
            logLOTREquipmentChanges,
            "Log each eligible newly spawned LOTR NPC whose configured equipment is applied, including its spawn source and resulting "
        ...
        lotrNPCWeaponRules = configuration
            .get(
                ...
                "Weighted weapon choices in the format target;itemName;weight. A target can be an exact LOTR NPC "
                    + "name, group:GROUP_NAME, faction:FACTION_CODE such as faction:GONDOR, or all. Use empty as an "
        ...
        lotrNPCRangedWeaponRules = configuration
            .get(
                ...
                "Weighted ranged-weapon choices in the format target;itemName;weight. A target can be an exact LOTR "
                    + "NPC name, group:GROUP_NAME, faction:FACTION_CODE such as faction:GONDOR, or all. Use empty as "
